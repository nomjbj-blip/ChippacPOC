...
namespace TestDataMigration
{
    public partial class FrmMain : Form
    {
        ...
        private void FrmMain_Load(object sender, EventArgs e)
        {
            ...
            btnRun.Enabled = btnStop.Enabled = false;
        }
        ...
        private void Process(object state)
        {
            ...
            while (date < dtEnd.Value && btnStop.Enabled)
            {
                ...
                foreach (DataRow lotRow in lotDt.Rows)
                {
                    ...
                    decimal mLotSeq = _mira.NewLotSeq();
                    ...
                    try
                    {
                        ...
                        foreach (DataRow wafRow in wafDt.Rows)
                        {
                            ...
                            foreach (DataRow tableRow in tableDt.Rows)
                            {
                                ...
                                start.FileName = "TestDataMigrationRawData.exe";
                                start.Arguments = String.Format("{0} {1} {2} {3}", Factory, tWafSeq, mWafSeq, tableRow[0]);
                        ...
                        // @TQP_WAFER INSERT
                        wafDt.Columns.Remove("PPD_SEQ");
                        wafDt.TableName = "TQP_WAFER";
                        _mira.InsertBulk(wafDt);
                        wafDt.Dispose();
        ...
        private void btnPROGRAM_PARM_DEF_Click(object sender, EventArgs e)
        {
            ...
            {
                DataTable paraDt = _test.GetProgramParameterByPpdSeq(Factory, row[0].ToString());
        ...
        private void btnXyToXY_Click(object sender, EventArgs e)
        {
            DataTable dt = _mira.Getdata_PARA_XY(Factory);
            AppendXAndYRow(dt);
            dt.TableName = "TQP_PARASPEC";
            _mira.InsertBulk(dt);
        }

        private void btnBINDESC_Click(object sender, EventArgs e)
        {
            DataTable binDt = _test.GetData_BINDESC(Factory);
            DataTable miraBinDt = EmptyBinTable();

            foreach (DataRow binRow in binDt.Rows)
            {
                string program = binRow["PROGRAM"].ToString();
