...
namespace TestDataMigration
{
    public partial class FrmMain2 : Form
    {
        ...
        private void ProcessLot(DateTime date, DataTable lotDt)
        {
            string message = String.Format("Date = {0}, Lot Count = {1}", date.ToString(DATE_FORMAT), lotDt.Rows.Count);
            Log.Info(message);
            AppendInfo(message);
            ...
            foreach (DataRow lotRow in lotDt.Rows)
            {
                ...
                try
                {
                    ...
                    Log.Info("Lot ID={0}, TW: {1}, M: {2}", lotID, tLotSeq, mLotSeq);
                    ...
                    for (int i = 0; i < wafDt.Rows.Count; i++)
                    {
                        ...
                        {
                            rawDataInfoList.Add(new RawDataInfo(tWafSeq, mWafSeq, tableRow[0].ToString()));
                        }
        ...
        class RawDataInfo
        {
            public RawDataInfo(decimal wafSeq, decimal mWafSeq, string tTableName)
            {
                WafSeq = wafSeq;
        ...
        private void RegParaspecAsync(object state)
        {
            ...
            try
            {
                ...
                int tableNameIndex = paraDt.Columns.IndexOf("TABLE_NAME");
                ...
                if (paraArr != null)
                {
                    foreach (string para in paraArr)
                    {
                        ...
                        {
                            string tTableName = dic[para][tableNameIndex].ToString(); // 이미 td_로 변환되어 있음
                            string mTableName = _mira.GetTableWhereField(program, para)[0];

                            if (tTableName != mTableName)
                            {
                                _mira.UpdateParaspecTable(program, para, tTableName);
                            }
                ...
                paraDt.Dispose();
                UpdateLastItem(program + " : " + paraDt.Rows.Count.ToString());
                AppendInfo(program);
            }
            finally
            {
        ...
        private void btnCreateTdTable_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                CreateTdTable(row["TABLE_NAME"].ToString());
            }
        }

        private void CreateTdTable(string tTableName)
        {
        ...
        private void btnSum_Click(object sender, EventArgs e)
        {
            ...
            {
                decimal lotSeq = decimal.Parse(txtSumLot.Text); //_mira.GetLotSeq(txtSumLot.Text);
                Summary(lotSeq);
                txtSumLot.Clear();
            }
        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            string[] tTableArr = _test.GetTableList(program);

            
            // WAFER_ID - T WAFER_SEQ - M WAFER_SEQ 매핑
            var tWaferSeqDic = _test.TTableSeqArr(program);
            var mWaferSeqDic = _mira.TTableSeqArr(program);

            foreach (var mItem in mWaferSeqDic)
            {
                foreach (var tItem in tWaferSeqDic)
                {
                    if (mItem.Value == tItem.Value)
                        txtInsertWafSeq.AppendText(String.Format("{0},{1}\n", tItem.Key, mItem.Key));
                }
            }

            //using (var sw = File.CreateText(String.Format(@"c:\{0}.txt", program)))
            //{
            //    foreach (var mItem in mWaferSeqDic)
            //    {
            //        foreach (var tItem in tWaferSeqDic)
            ...
            // 미라콤 td_data_tables 수정
            _mira.DeleteDataTables(program);
            _mira.InsertDataTables(Factory, program, tTableArr);
                        
            // 미라콤 테이블 DROP
            foreach (string tTable in tTableArr)
            {
                string mTable = ToMiracomTable(tTable);
                _mira.DropTable(mTable);

                // T 테이블 생성
                CreateTdTable(tTable);
            }
            ...
            while (date < dtEnd.Value)
            {
                ...
                foreach (DataRow lotRow in lotDt.Rows)
                {
                    ...
                    try
                    {
                        ...
                        DataTable dt = _mira.GetRawDataCountByWaferSeq(program, mLotSeq);
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("파일명 입력");
                return;
            }

            using (var sr = File.OpenText(textBox2.Text))
            {
                while (sr.Peek() > 0)
                {
        ...
        {
            DataTable dt = _test.GetLotSeqArrayFromTTable(textBox3.Text);

            ProcessLot(DateTime.Now, dt);
        }

        private void button7_Click(object sender, EventArgs e)
        {
            DataTable dt = _test.GetLotDataFromLotID(textBox4.Text, dateTimePicker1.Value.ToString("yyyy-MM-dd"));

            if (dt != null && dt.Rows.Count > 0)
                ProcessLot(dateTimePicker1.Value, dt);
        }

        private void button8_Click(object sender, EventArgs e)
        {
            decimal maxWaferSeq = _mira.GetMaxWaferSeq();
            DataTable wafSum = EmptyWaferSumTable();
            AppendInfo("max: " + maxWaferSeq.ToString());
            int add = 10000;

            for (decimal i = 1; i < maxWaferSeq + add; i += add)
            {
                UpdateLastItem(i.ToString());

                string[] arr = _mira.GetEmptyWaferSum(i, i + add);
                ...
                foreach (string val in arr)
                {
                    decimal waferSeq = decimal.Parse(val);
                    string ttable = _mira.GetBinTTable(waferSeq);
                    ...
                    decimal lotSeq = decimal.Parse(wafSumDt.Rows[0]["LOT_SEQ"].ToString());
                    string deviceAlias = wafSumDt.Rows[0]["DEVICE_ALIAS"].ToString();
                    string product = wafSumDt.Rows[0]["PRODUCT"].ToString();
                    ...
                    newWaf["FACILITY"] = wafSumDt.Rows[0]["FACILITY"];
                    newWaf["PRODUCT"] = 
                    newWaf["DEVICE_ALIAS"] = deviceAlias;
                    newWaf["TESTAREA"] = wafSumDt.Rows[0]["TESTAREA"];
                    newWaf["PROGRAM"] = wafSumDt.Rows[0]["PROGRAM"];
                    newWaf["MOTHER_LOT_ID"] = wafSumDt.Rows[0]["MOTHER_LOT_ID"];
                    newWaf["LOT_ID"] = wafSumDt.Rows[0]["LOT_ID"];
