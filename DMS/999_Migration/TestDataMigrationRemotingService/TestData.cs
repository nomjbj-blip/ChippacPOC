...
namespace TestDataMigrationRemotingService
{
    public class MyTestData
    {
        ...
        private static readonly string m_ESDA_ConnectionString = "Driver={Oracle in instantclient_10_2};Dbq=ANAMNTDB;Uid=t_test;Pwd=t_test;";
    ...
    public class TestData : IDisposable
    {
            ...
            )
        {
            ...
            // 임시로 probe_cnt > 0 인것만 처리 2019.11.25
            //sQuery.AppendLine("        AND T2.PROBE_CNT = 0 ");
            sQuery.AppendLine("        AND T2.PROBE_CNT > 0 ");
        ...
        public string[] GetTableListFromWaferSeq(decimal wafSeq)
        {
            string query = String.Format(@"
SELECT TABLE_NAME FROM DATA_TABLES
WHERE PROGRAM IN (
SELECT PROGRAM FROM LOT
    WHERE LOT_SEQ IN (
        SELECT LOT_SEQ FROM WAFER
        WHERE WAFER_SEQ = {0}
    )
) ORDER BY 1", wafSeq);
...
", factory));
        ...
        public DataTable GetProgramTable(string program)
        {
            ...
            dic.Add(":PROGRAM", new Parameter(DbType.String, program));
...
", tableName));
        ...
        public void GetTTableData(string tTableName, decimal[] waferSeqArr, out DataTable schemaTable, out List<object[]> dataList)
        {
            ExecuteQueryByTable(tTableName, waferSeqArr, out schemaTable, out dataList);
        }

        [System.Security.SecurityCritical]
        [System.Runtime.ExceptionServices.HandleProcessCorruptedStateExceptions]
        public void ExecuteQueryByTable(string tableName, decimal[] waferSeqArr, out DataTable schemaTable, out List<object[]> dataList)
        {
            using (OracleCommand comm = new OracleCommand())
            {
                using (OracleConnection conn = new OracleConnection())
                {
                    ...
                    {
                        sbWhere.Append("WHERE WAFER_SEQ = " + waferSeqArr[0]);
                    }
                    else
                    {
                        sbWhere.Append("WHERE WAFER_SEQ IN ");
                        sbWhere.AppendFormat("({0})", String.Join<decimal>(",", waferSeqArr));
                    }

                    comm.CommandText = String.Format("SELECT * FROM {0}", tableName);
                    OracleDataReader reader = comm.ExecuteReader(CommandBehavior.SchemaOnly);
                    schemaTable = reader.GetSchemaTable();
                    reader.Dispose();

                    comm.CommandText = String.Format("SELECT COUNT(*) FROM {0} {1}", tableName, sbWhere.ToString());
                    int dataCount = (int)(decimal)comm.ExecuteScalar();
                    conn.Close();
                    ...
                    {
                        if (schemaTable.Rows[i][0].ToString() != "UNIT_START_TIME")
                            sb.AppendFormat("TRUNC({0},20){0},", schemaTable.Rows[i][0]);
                        else
                            sb.AppendFormat("{0},", schemaTable.Rows[i][0]);
                    }
                    ...
                    while (reader.Read())
                    {
                        for (int c = 0; c < schemaTable.Rows.Count; c++)
                        {
                            ...
                            catch (AccessViolationException aex)
                            {
                                ...
                                System.Diagnostics.Debug.Write("AccessViolationException: " + aex.Message);
                            }
                            catch (Exception ex)
                            {
                                if ((Type)schemaTable.Rows[c]["DataType"] == typeof(decimal))
        ...
        public DataTable ExecuteQueryByTable(string tableName, decimal[] waferSeqArr)
        {
            using (OracleCommand comm = new OracleCommand())
            {
                using (OracleConnection conn = new OracleConnection())
...
", factory == "FAB2" ? "=" : "<>"));
        ...
        public DataTable GetData_DATA_TABLES(string factory)
        {

            return ExecuteQuery(String.Format(
@"
SELECT '{0}' FACTORY, PROGRAM, TABLE_NAME
FROM DATA_TABLES
WHERE PROGRAM IN (
    SELECT DISTINCT PROGRAM FROM LOT
    WHERE FACILITY {1} 'PROBE1'
    AND END_TIME >= TO_DATE('20180101','YYYYMMDD')
)
", factory, factory == "FAB2" ? "=" : "<>"));
        }

        public DataTable GetData_BINDESC(string factory)
        {

            return ExecuteQuery(String.Format(
@"
SELECT PROGRAM, BIN, BIN_LABEL, BIN_COLOR, GECBINS
FROM PROGRAM_SETUP_BIN_TAGS
WHERE (PROGRAM, PS_SEQ) IN (
    SELECT PROGRAM, MAX(PS_SEQ) PS_SEQ 
    FROM PROGRAM_SETUP_BIN_TAGS
    WHERE PROGRAM IN (
        SELECT PROGRAM
...
", factory, factory == "FAB2" ? "=" : "<>"));
        ...
        public DataTable GetTTableName(string wafer_seq)
        {
            return ExecuteQuery(String.Format(
@"
SELECT TABLE_NAME FROM DATA_TABLES
...
", wafer_seq));
        ...
        public List<string> GetTableListWithXY()
        {
...
");
        ...
        public bool ExsitsXYField(string tableName)
        {
...
WHERE EXISTS(
    ...
    AND TABLE_NAME = '{0}'
        ...
        public List<string> GetWaferSeqList(decimal tLotSeq)
        {
            return GetStringList(String.Format(
@"
SELECT WAFER_SEQ FROM WAFER
WHERE LOT_SEQ = {0}
", tLotSeq));
        }

        public DataTable GetXYData(string tableName, string waferSeq)
        {
            DataTable dt = new DataTable();

            using (OracleCommand comm = new OracleCommand())
            {
                using (OracleConnection conn = new OracleConnection(m_ESDA_ConnectionString))
                {
                    comm.Connection = conn;
                    comm.CommandType = CommandType.Text;
                    comm.CommandText = String.Format("SELECT 0 WAFER_SEQ, WAFER_SEQ OLD_WAFER_SEQ, DIE_NUM, 0 X, XY Y FROM {0} WHERE WAFER_SEQ = :WAFER_SEQ", tableName);

                    OracleParameter para = new OracleParameter(":WAFER_SEQ", Decimal.Parse(waferSeq));
        ...
        public Dictionary<decimal, int> GetLotAndWafCnt(string factory, DateTime dStartDate, DateTime dEndDate)
        {
            DataTable dt = ExecuteQuery(String.Format(
@"
SELECT A.LOT_SEQ, COUNT(*) CNT
FROM LOT A, WAFER B
WHERE A.LOT_SEQ = B.LOT_SEQ
AND A.END_TIME >= TO_DATE('{0}', 'YYYY-MM-DD')
AND A.END_TIME < TO_DATE('{1}', 'YYYY-MM-DD')
AND FACILITY {2} 'PROBE1'
AND B.PROBE_CNT = 0
GROUP BY A.LOT_SEQ
ORDER BY A.LOT_SEQ
", dStartDate.ToString("yyyy-MM-dd"), dEndDate.ToString("yyyy-MM-dd"), (factory == "FAB2") ? "=" : "<>"));
        ...
        public DataTable GetProgramParameterByProgram(string Factory, string program)
        {
            ...
            return ExecuteQuery(String.Format(
                ...
                "    0 AS \"PARAM_INDEX\", " +
                "    PARM AS \"PARAM_NAME\", " +
                "    PARM_TYPE AS \"PARAM_TYPE\", " +
                "    PARM_DESC AS \"PARAM_DESC\", " +
                "    SYSDATE CREATE_TIME, " +
                "    'MIGRATION' AS \"CREATE_USER\", " +
                "    SYSDATE UPDATE_TIME, " +
                "    'MIGRATION' AS \"UPDATE_USER\", " +
                "    'TD_'||SUBSTR(TABLE_NAME,2) AS \"TABLE_NAME\", " +
                "    0 AS \"DECIMAL_PLACES\", " +
                "    RUNTIME_DEFINED AS \"RUNTIME_DEFINED\", " +
                "    UOM, " +
                "    SPEC_LOW AS \"LSL\", " +
                "    TARGET AS \"TARGET\", " +
                "    SPEC_HI AS \"USL\", " +
                ...
                " FROM (" + 
                "   SELECT A.* " + 
                "   FROM PROGRAM_PARM_DEF A, (" + 
                "       SELECT PROGRAM, PARM, MAX(PPD_SEQ) PPD_SEQ" +
                "       FROM PROGRAM_PARM_DEF" + 
                "        GROUP BY PROGRAM, PARM" + 
                "    )B" + 
                "    WHERE A.PROGRAM = B.PROGRAM" + 
                "        AND A.PARM = B.PARM" + 
                "        AND A.PPD_SEQ = B.PPD_SEQ" + 
                "        AND A.PROGRAM = '{1}'" + 
                "        AND A.TABLE_NAME IS NOT NULL" + 
                " )" + 
                " ORDER BY PARAM_INDEX", Factory, program));
        }
        ...
        public Dictionary<string, string> TTableSeqArr(string program)
        {
            DataTable dt = ExecuteQuery(String.Format(@"
SELECT DISTINCT 
    (SELECT WAFER_ID FROM WAFER WHERE WAFER_SEQ = A.WAFER_SEQ) WAFER_ID,
    WAFER_SEQ
FROM T{0} A", program));
        ...
        public DataTable GetLotSeqArrayFromTTable(string tTableName)
        {
            return ExecuteQuery(String.Format(@"
SELECT LOT_SEQ,
	LOT_SEQ OLD_LOT_SEQ,
	...
	SMS_ITEM_ID,
	FAMILY,
	DECODE(FACILITY, 'PROBE1', 'FAB2', 'FAB1') AS FACTORY,
	START_TIME,
	END_TIME,
	WAFER_DIAM,
	COMMENTS,
...
FROM LOT
WHERE LOT_SEQ IN (
    SELECT DISTINCT LOT_SEQ FROM WAFER
    WHERE WAFER_SEQ IN (
    SELECT WAFER_SEQ FROM {0}
    )
)
", tTableName));
        }

        public DataTable GetLotDataFromLotID(string lotID, string date)
        {
            return ExecuteQuery(String.Format(@"
SELECT LOT_SEQ,
	LOT_SEQ OLD_LOT_SEQ,
...
FROM LOT
WHERE LOT = '{0}'
AND TO_CHAR(END_TIME,'YYYY-MM-DD') > '{1}'
", lotID, date));
        }
    }
}
