...
namespace DACrux.Map
{
	public class Zonal : System.Windows.Forms.UserControl
	{
		...
		/// Region 속성
		/// </summary>
		protected double m_dNotchSize = 3.0d;
...
//													   69.0f,75.3f,90.0f,80.0f,72.8f,68.7f,78.0f,82.0f,100.0f};
		...
		protected PointD GetRealPoint(int piX,int piY)
		{
            ...
            pdCenter.X = m_WaferRecipe.WAFER_RADIUS - (m_rectdWaferArea.X + piX / (m_dZoomRatio * m_dScale));
            pdCenter.Y = -m_WaferRecipe.WAFER_RADIUS + (m_rectdWaferArea.Y + piY / (m_dZoomRatio * m_dScale));
			return pdCenter;
		}
		...
		private PointF GetViewPoint(double dX,double dY)
		{
            ...
            pfCenter.X = (float)((-m_rectdWaferArea.X + m_WaferRecipe.WAFER_RADIUS + dX) * (m_dZoomRatio * m_dScale));
            pfCenter.Y = (float)((-m_rectdWaferArea.Y + m_WaferRecipe.WAFER_RADIUS - dY) * (m_dZoomRatio * m_dScale));
			return pfCenter;
		}
		#endregion
		...
		protected virtual void DrawWafer()
		{
			...
			try
			{
				...
				if(m_WaferRecipe.NOTCH_TYPE != Notch.Notch)
				{
					...
					gpWafer.AddLine((float)((- m_rectdWaferArea.X + dNotchStart.X + (m_WaferRecipe.WAFER_RADIUS)) * (m_dZoomRatio * m_dScale))
                        , (float)((-m_rectdWaferArea.Y + (m_WaferRecipe.WAFER_RADIUS) - dNotchStart.Y) * (m_dZoomRatio * m_dScale))
                        , (float)((-m_rectdWaferArea.X + dNotchEnd.X + (m_WaferRecipe.WAFER_RADIUS)) * (m_dZoomRatio * m_dScale))
                        , (float)((-m_rectdWaferArea.Y + (m_WaferRecipe.WAFER_RADIUS) - dNotchEnd.Y) * (m_dZoomRatio * m_dScale)));
				}
				///==================================================================================================
				///
				...
				if(m_WaferRecipe.NOTCH_TYPE == Notch.Notch)
				{
                    ...
                    gpWafer.AddArc((float)((-m_rectdWaferArea.X + dNotchRStart.X + (m_WaferRecipe.WAFER_RADIUS)) * (m_dZoomRatio * m_dScale))
                        , (float)((-m_rectdWaferArea.Y - dNotchRStart.Y + m_WaferRecipe.WAFER_RADIUS - (dNotchREnd.X - dNotchRStart.X) / 2) * (m_dZoomRatio * m_dScale))
				...
				m_gdiTempMap.DrawArc(pEdge
                    , (float)((-m_rectdWaferArea.X + (m_WaferRecipe.WAFER_RADIUS) - (m_WaferRecipe.WAFER_SIZE / 2) * 1 / 4) * (m_dZoomRatio * m_dScale))
                    , (float)((-m_rectdWaferArea.Y + (m_WaferRecipe.WAFER_RADIUS) - (m_WaferRecipe.WAFER_SIZE / 2) * 1 / 4) * (m_dZoomRatio * m_dScale))
				...
				m_gdiTempMap.DrawArc(pEdge
                    , (float)((-m_rectdWaferArea.X + (m_WaferRecipe.WAFER_RADIUS) - (m_WaferRecipe.WAFER_SIZE / 2) * 2 / 4) * (m_dZoomRatio * m_dScale))
                    , (float)((-m_rectdWaferArea.Y + (m_WaferRecipe.WAFER_RADIUS) - (m_WaferRecipe.WAFER_SIZE / 2) * 2 / 4) * (m_dZoomRatio * m_dScale))
				...
				///
				m_gdiTempMap.DrawArc(pEdge
                    , (float)((-m_rectdWaferArea.X + (m_WaferRecipe.WAFER_RADIUS) - (m_WaferRecipe.WAFER_SIZE / 2) * 3 / 4) * (m_dZoomRatio * m_dScale))
                    , (float)((-m_rectdWaferArea.Y + (m_WaferRecipe.WAFER_RADIUS) - (m_WaferRecipe.WAFER_SIZE / 2) * 3 / 4) * (m_dZoomRatio * m_dScale))
		...
		private void CalSelectedDie(GraphicsPath SelectPath,ref int GecCount,ref int TotalCount)
		{
			...
			try
			{
				foreach(Die InDie in m_arrDies)
				{
					...
					RotatePoint(ref pdDieCenter.X,ref pdDieCenter.Y,270);

					PointF pfPoint = PointF.Empty;
                    pfPoint.X = (float)((-m_rectdWaferArea.X + m_WaferRecipe.WAFER_RADIUS + pdDieCenter.X) * (m_dZoomRatio * m_dScale));
                    pfPoint.Y = (float)((-m_rectdWaferArea.Y + m_WaferRecipe.WAFER_RADIUS - pdDieCenter.Y) * (m_dZoomRatio * m_dScale));
                    if (SelectPath.IsVisible(pfPoint))
                    {
                        //if (Array.IndexOf(m_strSelectedBin, m_strGoodBin) > -1)
                        //    GecCount++;

                        if(InDie.BinNumber == int.Parse(m_strGoodBin))
                            GecCount++;
