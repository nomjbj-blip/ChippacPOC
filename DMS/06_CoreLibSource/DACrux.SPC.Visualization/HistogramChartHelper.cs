...
namespace DACrux.SPC.Visualization
{
    ...
    public class HistogramChartHelper
    {
        ...
        public void ORG__CreateHistogram(
            Chart chartControl,
            ...
            string histogramSeriesName)
        {
            ...
            double mean = chartControl.DataManipulator.Statistics.Mean(dataSeriesName);
            double variance = chartControl.DataManipulator.Statistics.Variance(dataSeriesName, true);
            double stdev = Math.Sqrt(variance);
            double minValue = mean - stdev * 6;
            double maxValue = mean + stdev * 6;
            int pointCount = 0;
            ...
            for (currentPosition = minValue; currentPosition <= maxValue; currentPosition += this.SegmentIntervalWidth)
            {
                ...
                dFx = (currentPosition - minValue) * this.SegmentIntervalWidth / (maxValue - minValue) + currentPosition;
                dFy = GetdNormal(dFx, mean, stdev);
                dNormal = GetdNormal(mean, mean, stdev);
            ...
            string histogramSeriesName)
        {
            ...
            for (currentPosition = minValue; currentPosition <= maxValue; currentPosition += this.SegmentIntervalWidth)
            {
                ...
                //dFx = (currentPosition - minValue) * this.SegmentIntervalWidth / (maxValue - minValue) + minValue;
                //dFy = GetdNormal(dFx, mean, stdev);
                //dNormal = GetdNormal(mean, mean, stdev);

                chartControl.Series["NormalDistribution"].Points.AddXY(currentPosition + this.SegmentIntervalWidth / 2.0, count);
        ...
        public static double GetdNormal(double dX, double dMu, double dSigma)
        {
            ...
            try
            {
                const double onebys2pi = 0.398942280401433;
