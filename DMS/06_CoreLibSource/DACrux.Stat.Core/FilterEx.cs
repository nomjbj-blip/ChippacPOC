 ...
 * 2015.3.26 : DACrux.Base.FilterEx 와 동일한 Class 임
 *             DACrux.Base를 DACrux.Stat.Core에 참조하지 않고 사용하기 위해 Internal Class로 
 *             이(DACrux.Stat.Core) Namespace에서 사용
    ...
    internal class FilterEx
    {
        ...
        public static void SelectMinMax(DataTable SourceTable, string FieldName, out double Min, out double Max)
        {
            try
            {
                ...
                Min = double.Parse(dr[0][FieldName].ToString());
                Max = double.Parse(dr[dr.Length - 1][FieldName].ToString());
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public static void SelectMinMax(DataTable SourceTable, string FieldName, out int Min, out int Max)
        {
            try
            {
                ...
                Min = int.Parse(dr[0][FieldName].ToString());
                Max = int.Parse(dr[dr.Length - 1][FieldName].ToString());
            }
            catch (Exception ex)
            {
                throw ex;
