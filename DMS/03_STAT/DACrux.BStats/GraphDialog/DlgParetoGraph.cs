...
{
    public partial class DlgParetoGraph : Form, iGraphAnalysis
    {

        #region " MEMBER FIELD "

        public string FUNC_CODE = "F0406";
        ...
        public DlgParetoGraph(DataTable dataSource, List<DACrux.ProjectManager.UI.DataView.ColumnInfo> lstValidColumnInfo)
            :this(dataSource, lstValidColumnInfo, "Graph Analysis")
        {
        }

        public DlgParetoGraph(DataTable dataSource, List<DACrux.ProjectManager.UI.DataView.ColumnInfo> lstValidColumnInfo, string createdInformation)
        {
            InitializeComponent();

            graphInfo = new GraphInformation();
            graphInfo.Type = GraphType.Pareto;
        ...
        public DlgParetoGraph(GraphInformation graphInfo)
        {
            InitializeComponent();
        ...
        private void btnRight_Ser_Click(object sender, EventArgs e)
        {
            if (lvColumns.SelectedItems.Count > 0)
            {
                ...
                if(filteredItems.Count > 0)
                {
                    ...
                    DACrux.ProjectManager.UI.Common.MoveListViewItem(lvColumns, lvSeries, filteredItems[0]);
                }
            }
        }
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            if (lvSeries.Items.Count != 1)
                strMessage = "Pareto Graph needs only one series.";
            ...
            #region [ Sorting Pareto Style ]

            System.Data.DataView dv = graphInfo.DataSource.DefaultView;
            dv.Sort = graphInfo.DataSource.Columns[((GraphInformation.ColumnInfoItem)htColumns[lvSeries.Items[0]]).ColumnIndex].ColumnName + " DESC";
            graphInfo.DataSource = dv.ToTable();
            graphInfo.DataSource.AcceptChanges();
