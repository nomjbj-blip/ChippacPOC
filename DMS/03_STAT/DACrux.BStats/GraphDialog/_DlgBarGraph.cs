...
{
    public partial class _DlgBarGraph : Form, iGraphAnalysis
    {
        #region " MEMBER FIELD "
        ...
        public _DlgBarGraph(DataTable dataSource, List<DACrux.ProjectManager.UI.DataView.ColumnInfo> lstValidColumnInfo) :this(dataSource, lstValidColumnInfo, "Graph Analysis")
        {
        }

        public _DlgBarGraph(DataTable dataSource, List<DACrux.ProjectManager.UI.DataView.ColumnInfo> lstValidColumnInfo, string createdInformation)
        {
            InitializeComponent();
        ...
        public _DlgBarGraph(GraphInformation graphInfo)
        {
            InitializeComponent();
        ...
        private void InitDialog()
        {
            ...
            this.btnRight_Val.Click += new EventHandler(btnRight_Val_Click);
            this.btnLeft_Val.Click += new EventHandler(btnLeft_Val_Click);
            ...
            this.lvValue.MouseDoubleClick += new MouseEventHandler(lvValue_MouseDoubleClick);
            ...
            this.lvValue.ItemDrag += new ItemDragEventHandler(lvValue_ItemDrag);
            ...
            this.lvValue.DragEnter += new DragEventHandler(lvValue_DragEnter);
            ...
            this.lvValue.DragDrop += new DragEventHandler(lvValue_DragDrop);
        ...
        private void SetValueListView()
        {
            ...
            {
                if (((GraphInformation.ColumnInfoItem)htColumns[lvColumnItem]).Equals(graphInfo.Series))
                {
                    DACrux.ProjectManager.UI.Common.MoveListViewItem(lvColumns, lvValue, lvColumnItem);
                    break;
                }
            }
        }
        ...
        void lvValue_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            if (lvValue.SelectedItems.Count > 0)
            {
                DACrux.ProjectManager.UI.Common.MoveListViewItem(lvValue, lvColumns, lvValue.SelectedItems[0]);
            }
        }
        ...
        void lvColumns_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            if (lvColumns.SelectedItems.Count > 0)
            {
                ...
                if (columnInfo.ColumnType == typeof(double))
                    DACrux.ProjectManager.UI.Common.MoveListViewItem(lvColumns, lvValue, lvColumns.SelectedItems[0]);
                else
                {
        ...
        private void btnRight_Val_Click(object sender, EventArgs e)
        {
            if (lvColumns.SelectedItems.Count > 0)
            {
                ...
                DACrux.ProjectManager.UI.Common.MoveListViewItems(lvColumns, lvValue, (IList)filteredItems);
            }
        }

        private void btnLeft_Val_Click(object sender, EventArgs e)
        {
            if (lvValue.SelectedItems.Count > 0)
                DACrux.ProjectManager.UI.Common.MoveListViewItems(lvValue, lvColumns, (IList)lvValue.SelectedItems);
        }
        ...
        void lvValue_ItemDrag(object sender, ItemDragEventArgs e)
        {
            lvDragSource = (ListView)sender;
            lvDragSource.DoDragDrop(lvDragSource.SelectedItems, DragDropEffects.Move);
        }
        ...
        void lvValue_DragEnter(object sender, DragEventArgs e)
        {
            if (lvDragSource != null && lvDragSource != lvValue)
                e.Effect = DragDropEffects.Move;
            else
                e.Effect = DragDropEffects.None;
        }
        ...
        void lvValue_DragDrop(object sender, DragEventArgs e)
        {
            if (e.Data.GetDataPresent(typeof(ListView.SelectedListViewItemCollection)) && e.Effect == DragDropEffects.Move)
            {
                IList items = (IList)e.Data.GetData(typeof(ListView.SelectedListViewItemCollection));
                ...
                DACrux.ProjectManager.UI.Common.MoveListViewItems(lvDragSource, (ListView)sender, filteredItems);
            }
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            if (lvValue.Items.Count < 1)
                strMessage = "At least one column should be selected.";

            if (lvLabel.Items.Count != 1)
                strMessage = "Specify a column for Axis X Label";

            if (lvSeries.Items.Count > 0 && lvValue.Items.Count > 1)
                strMessage = "Only one Value column avaiLabel when a Series column is set.";

            for (int i = 0; i < lvValue.Items.Count; i++)
            {
                if (((GraphInformation.ColumnInfoItem)htColumns[lvValue.Items[i]]).ColumnType != typeof(double))
                {
            ...
            if (lvSeries.Items.Count > 0)
                graphInfo.Series = (GraphInformation.ColumnInfoItem)htColumns[lvSeries.Items[0]];
            else
                graphInfo.Series = new GraphInformation.ColumnInfoItem(-1, null, null, null);

            foreach (ListViewItem lvItem in lvValue.Items)
        ...
        private void _DlgBarGraph_Activated(object sender, EventArgs e)
        {
        ...
        private void _DlgBarGraph_KeyUp(object sender, KeyEventArgs e)
        {
