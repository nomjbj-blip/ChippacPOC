...
namespace DACrux.TEST.BSL
{
    ...
    public class TestDataManager
    {
        ...
        private static readonly string FAB1 = "FAB1";
        private static readonly string PARAMETRIC = "PARAMETRIC";
            ...
            )
        {
            ...
            TQP_PROGRAM pgm = new TQP_PROGRAM();
            DataTable dtPgm = pgm.GetProgramInfo(programName);
            if (dtPgm != null && dtPgm.Rows.Count > 0)
            {
                testarea = dtPgm.Rows[0]["TESTAREA"].ToString();
                deviceAlias = dtPgm.Rows[0]["DEVICE"].ToString();
            }
            ...
            {
                // 해당 프로그램에 대해 신규 입력 인 경우, 신규 프로그램에 대한 테이블 생성
                tableArr = CreateNewTable(factory, programName, programRev, columnList);
                tableList.AddRange(tableArr);

                /// FAB1 TestArea: PARAMETRIC
                /// Normal Test 진행 시 Retest Program 에 대한 DB Table 도 함께 만든다.
                if (String.Equals(factory, FAB1) && String.Equals(testarea, PARAMETRIC))
                {
                    /// 프로그램의 맨 마지막 자리가 "N"인 경우, Full map은 제외
                    string wildCard = programName.Substring(programName.Length - 1, 1);
                    if (!programName.Contains("FUL") && String.Equals(wildCard, "N"))
                    {
                        string newProgramName = String.Format("{0}R", programName.Substring(0, programName.Length - 1));
                        tableArr = obj.GetTableNames(factory, newProgramName);
                        if (tableArr == null || tableArr.Length == 0)
                        {
                            pgm.MergeData(factory, newProgramName, testarea, deviceAlias);
                            CreateNewTable(factory, newProgramName, programRev, columnList);
                        }
                    }
                }
            }
            else
            {
                ...
                /// FAB1, Parametric 의 경우 테이블의 동기화 한다.
                #region [ FAB1 PCM 에 대한 테이블 컬럼 추가 로직 ]
                List<string> newTableList = new List<string>();
                if (String.Equals(factory, FAB1) && String.Equals(testarea, PARAMETRIC))
                {
                    string newProgramName = string.Empty;
                    string wildCardKey = programName.Substring(programName.Length - 1, 1);
                    /// Normal test에 대한 Program 일 경우
                    if (!programName.Contains("FUL") && String.Equals(wildCardKey, "N"))
                    {
                        // 올라온 프로그램 이름에 대한 R/T 프로그램 명으로 치환
                        newProgramName = String.Format("{0}R", programName.Substring(0, programName.Length - 1));
                        pgm.MergeData(factory, newProgramName, testarea, deviceAlias);
                        // R/T 프로그램명에 대한 Table List 체크
                        tableArr = obj.GetTableNames(factory, newProgramName);
                        newTableList.Clear();
                        if (tableArr != null && tableArr.Length > 0)
                            newTableList.AddRange(tableArr);

                        programRev = par.GetMaxProgramRev(factory, newProgramName);
                        // 신규 생성
                        if (newTableList.Count == 0)
                        {
                            tableArr = CreateNewTable(factory, newProgramName, programRev, columnList);
                            newTableList.AddRange(tableArr);
                        }
                        else
                        {
                            // 기존 테이블에 컬럼 추가
                            CheckColumns(factory, newProgramName, programRev, newTableList, columnList);
                        }
                    }
                    /// Retest에 대한 Program 일 경우
                    if (!programName.Contains("FUL") && String.Equals(wildCardKey, "R"))
                    {
                        // 올라온 프로그램 이름에 대한 R/T 프로그램 명으로 치환
                        newProgramName = String.Format("{0}N", programName.Substring(0, programName.Length - 1));
                        pgm.MergeData(factory, newProgramName, testarea, deviceAlias);
                        // N/T 프로그램에 대한 Table List 체크
                        tableArr = obj.GetTableNames(factory, newProgramName);
                        newTableList.Clear();
                        if (tableArr != null && tableArr.Length > 0)
                            newTableList.AddRange(tableArr);
                ...
                #endregion [ FAB1 Parameteric 에 대한 테이블 컬럼 추가 로직 ]
            }

            using (TD_TABLE tbl = new TD_TABLE())
            {
            ...
            )
        {
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < rawDt.Rows.Count; i++)
                {
                    SetData(ds, dtParamSpec, rawDt.Rows[i], waferSeq, i);
                }
            }
            else // 최초 등록시 
            {
                for (int i = 0; i < rawDt.Rows.Count; i++)
                {
                    SetData(ds, dtParamSpec, rawDt.Rows[i], waferSeq, i + 1, 0);
                }
            }
            ...
            )
        {
            ...
            foreach (DataColumn col in row.Table.Columns)
            {
                ...
                for (int i = 0; i < ds.Tables.Count; i++)
                {
                    ...
                    {
                        // DB Data
                        if (ds.Tables[i].Rows.Count <= iDieNum)
                            continue;
