...
namespace DACrux.TEST.Control
{
	...
	public class ChartDataTableHelper
	{
        ...
        protected bool AddTableHeader = false;
		...
		/// set a boolean to show or hide total columns.
		/// </summary>
        public void Initialize(System.Windows.Forms.DataVisualization.Charting.Chart chartObj, bool addTableTotals, bool addTableHeader = false)
		{
		...
		public void AddDataTable(string chartAreaName)
		{
			...
			if(AddTableTotals)
			{
				...
				ChartObj.Series.Add("Total");
				ChartObj.Series["Total"].ChartArea = chartAreaName;
				ChartObj.Series["Total"].Enabled = false;
				ChartObj.Series["Total"].Color = Color.Gainsboro;
			}
				...
				if(chartAreaName == ser.ChartArea)
				{
                    ...
                    foreach (DataPoint dp in ser.Points)
                    {
                        ...
                        ChartObj.ChartAreas[chartAreaName].AxisX.CustomLabels.Add(
                            From, To,
                            sLable,     // space used as a placeholder
		...
		private void AdjustXValues(System.Windows.Forms.DataVisualization.Charting.Series series)
		{
			...
			if(series.Name == "Total")
				return;
			else if(ChartObj.Series["Total"].Points.Count > 0)
			...
			foreach(DataPoint pt in series.Points)
			{
				if(AddDummyPoints)
				{
					ChartObj.Series["Total"].Points.AddXY(pt.XValue, pt.YValues[0]);
				}
				else
				{
					ChartObj.Series["Total"].Points[index].YValues[0] += pt.YValues[0];
				}
				...
				for(int i = 0; i < e.Chart.Series.Count; i++)
				{
					if(area.Name == e.Chart.Series[i].ChartArea)
					{
						...
						for(double AxisValue = min; AxisValue < max; AxisValue++)
						{
                            ...
                            //point1.Y = pixelY;
                            point2.Y = ChartAreaBottomY - titleFontSize.Height + (AddTableHeader ? axisFont.Height : 0);
                            point1.Y = pixelY + (AddTableHeader ? axisFont.Height : 0);
							...
							foreach(Series ser in ChartObj.Series)
							{
								if(area.Name == ser.ChartArea)
								{
									if(!TableLegendDrawn)
									{
                                        ...
                                        e.ChartGraphics.Graphics.FillRectangle(new SolidBrush(ser.Color),
                                            rect.X, row * (axisFont.Height) + (point1.Y), 10, axisFontSize.Height);

                                        RectangleF textRect = new RectangleF(rect.X + 10, row * (axisFont.Height) + (point1.Y), x - (rect.X + 10), axisFontSize.Height);
                                        e.ChartGraphics.Graphics.DrawString(ser.Name, axisFont, new SolidBrush(area.AxisX.LabelStyle.ForeColor), textRect, format);

										e.ChartGraphics.Graphics.DrawRectangle(new Pen(borderColor),
                                            rect.X, row * (axisFont.Height) + (point1.Y), x - rect.X, axisFontSize.Height);
