...
void RptWaferId_DAC()
{
    ...
    LOT *lot;
    WAFER *wafer;
    char err_msg[1024], logfile[100], tst_name[100], buf[100], filename[30];
    time_t logtime;
    int stat;
    int idx;
    ...
    sprintf(filename, "%s_%s_%.2d%.2d.txt", lot->id, wafer->id, timestruct->tm_min, timestruct->tm_sec);
    sprintf(logfile, "/opt/kiS600/ap_data/EQUIP/TEST/PCM_TMP/%s", filename);
