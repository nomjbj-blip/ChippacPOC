...
void RptWaferId_DAC()
{
    ...
    LOT *lot;
    WAFER *wafer;
    char err_msg[1024], logfile[100], tstPgm[100], tst_name[100], buf[100], srcFileName[50];
    char *filename = NULL;
    time_t logtime;
    int stat;
    int idx;
    ...
    for(idx = 0; idx < strlen(tst_name); idx++)
    {
        if(tst_name[idx] == '_')
        {
            tstPgm[idx] = NULL;
        ...
        tstPgm[idx] = tst_name[idx];
    }

    RptEquipStat("WAFER_START", lot->id, wafer->id, tstPgm);
