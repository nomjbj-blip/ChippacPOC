...
struct tm *tm_p;
 ...
 /* To get time format : YYYY/MM/DD HH:MM:SS */
	...
	for(i=0;i<strlen(yeardate); i++) if(yeardate[i] == ' ') yeardate[i] = '0';
	yeardate[30] = '\0';
	printf("YYYYMMDD == %s\n",yeardate);

 /* To get FILE time format : HHMMSS */
	sprintf(time,"%2d:%2d:%2d", tm_p->tm_hour, tm_p->tm_min, tm_p->tm_sec);
	for(i=0;i<strlen(time); i++) if(time[i] == ' ') time[i] = '0';
	time[30] = '\0';
	printf("HH:MM:SS == %s\n",time);
...
{                                                     /* BEGIN FUNCTION      */
  ...
  fprintf(output_file, "-------------------------------------\n");
  fprintf(output_file, "Current Die = %s,   Current Die Type = %s\n\n",
	curr_die(), curr_die_type());
...
int eot_text_inner()
{
   ...
   char *localPath = format_directory_name();
   char script[1024];
   
   struct dirent *dirEntry;
   
   while (dirEntry = readdir(localPath))
   {
   		/* exists directory: . ..  */
        ...
        /* Copy file */
    	sprintf(script, "cp %s/%s %s/%s", localPath, dirEntry->d_name, nfsPath, dirEntry->d_name);
        system(script);
        
        /* Delete file */
        sprintf(script, "rm %s%s", localPath, dirEntry->d_name);
        system(script);
   }
