...
namespace MigrationTools.ESDA
{
    public class HFtpClient
    {
        ...
        public HFtpClient(string ip, int port, string user, string pw, bool bUsePassive = true, bool bKeepAlive = true)
        {
            m_ip = ip;
            m_port = port;
            m_user = user;
            m_pw = pw;
            m_bUsePassive = bUsePassive;
        ...
        public HFtpContent[] Dir()
        {
            ...
            try
            {
                ...
                ftp.UsePassive = m_bUsePassive;
                ftp.KeepAlive = m_bKeepAlive;
                ...
                for (int a = 0; a < item.Length; a++)
                {
                    ...
                    else
                    {
                        ...
                        ftpContent.dateTime = DateTime.ParseExact(item[a].Substring(0, 17)
                            , "MM-dd-yy  hh:mmtt", System.Globalization.CultureInfo.InvariantCulture);
                        ftpContent.length = long.Parse(item[a].Substring(17, 21).Trim());
                    }
        ...
        public HFtpContent[] DirSimpleFileList()
        {
            WebResponse response = null;
            Stream stream = null;
            System.Text.StringBuilder sb = null;
            ...
            try
            {
                ...
                for (int a = 0; a < item.Length; a++)
                {
                    ...
                    string[] strFile = item[a].Split(new char[] { '/' }, StringSplitOptions.RemoveEmptyEntries);

                    if (strFile.Length == 1)
                        ftpContent.name = strFile[0];
                    else
                    {
                        if (strFile.Length < 2) continue;

                        ftpContent.name = strFile[1];
                    }
                    ftpContent.type = HFtpContentType.FILE;
